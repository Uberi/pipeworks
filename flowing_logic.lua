...
local pipe_liquid_max = 3
local pipe_liquid_shows_loaded = 0.5

if finite_liquids then
	pipe_liquid_max = 16
	pipe_liquid_shows_loaded = 4
end		
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if table.getn(water_nodes) > 1 then
			if string.find(node_above.name, "pipeworks:pipe_") then
				if node_level_above < pipe_liquid_max then
					minetest.get_meta(pos_above):set_float("liquid_level", node_level_above + 1)
...
-- A pump that's turned off should slowly leak down the pressure on the pipe

minetest.register_abm({
	nodenames = {"pipeworks:pump_off"},
	interval = 1,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		local pos_above = {x = pos.x, y = pos.y+1, z = pos.z}
		...
		if string.find(node_above.name, "pipeworks:pipe_") then
			if node_level_above > pipe_liquid_shows_loaded then
				minetest.get_meta(pos_above):set_float("liquid_level", node_level_above - pipe_liquid_shows_loaded)
			else
				minetest.get_meta(pos_above):set_float("liquid_level", 0)
			end
			if node_level_above <= pipe_liquid_shows_loaded then
				if string.find(node_above.name, "pipeworks:pipe_*loaded") then
					local newnode = string.gsub(node_above.name, "loaded", "empty")
					minetest.swap_node(pos_above, {name=newnode, param2 = node.param2}) 
				end
			end
		end
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if node_level_below then
			...
			elseif node_level_below <= 1 then
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if below_node.name == "air" or below_node.name == "default:water_flowing" or below_node.name == "default:water_source" then 
			...
			if string.find(adjacent_node.name, "pipeworks:pipe_") then
				...
				elseif adjacent_node_level <= 1 then
...
--[[
other nodes that need processed manually:
